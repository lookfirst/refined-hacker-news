...
export function createOptionsBar(position) {
  ...
  if (position === "top") {
    ...
    const itemlistTableParent = document.querySelector("table.itemlist")
      .parentElement;
    const itemlistTableBody = document.querySelector("table.itemlist > tbody");
    itemlistTableParent.insertBefore(
      optionsBar,
      itemlistTableBody.nextElementSibling
    );
  }
...
export function getGroupedStoriesItemList(itemlist) {
  ...
  for (let i = 0; i < rows.length - 2; i += 3) {
    ...
    const commentsLink = [...rows[i + 1].querySelectorAll("a")].find(
      (a) => a.innerText.includes("comment") || a.innerText.includes("discuss")
    );
    const commentsCount = commentsLink
      ? parseInt(commentsLink.innerText, 10)
      : undefined;

    const elements = [rows[i], rows[i + 1], rows[i + 2]];
